...
export const createNetwork = (peerjs: PeerJs, siteId: string): Network => {
  ...
  peerjs.on("open", async (id: string) => {
    const peer: Peer = { siteId: siteId, peerId: id, avatar: AvatarMapInterface.peerIdToAvatar(id) };
    ...
    eventBus.on("addToNetwork", ({ peerToBeAdded, peerSender, networkVersion }) => {
      ...
      eventBus.emit("broadcastAddToNetwork", {
        peerToBeAdded: peerToBeAdded,
