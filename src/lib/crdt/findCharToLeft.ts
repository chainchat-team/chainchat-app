...
export function findCharToLeft(crtd: Crdt, editor: Editor, point: Point): Char {
  ...
  if (characters === undefined) {
    throw new Error(
      `Unable to get left char. Characters are not defined for leaf node at point ${JSON.stringify(point)}`
    );
  }
  ...
  if (characters.length === 0) {
    ...
    throw new Error(
      `Unable to get left char. Characters length is ${characters.length} for leaf node at point ${JSON.stringify(
        point
      )}`
    );
  }
